/**
 * Autenticação do módulo /tecnico (PWA dos mecânicos).
 *
 * - PIN de 6 dígitos armazenado como scrypt:<salt_b64>:<hash_b64>.
 * - Cookie `tecnico_session` = <payload_b64url>.<sig_b64url>
 *   payload: <tecnico_id>.<exp_ms>.<nonce>
 *   sig:     HMAC-SHA256(payload, TECNICO_SESSION_SECRET)
 * - Rate limit: 5 falhas em 15 min por IP via tabela tecnico_login_attempts.
...
export const TECNICO_COOKIE = 'tecnico_session';
...
function getSessionSecret(): string {
  ...
  return (
    process.env.TECNICO_SESSION_SECRET ||
    'dev-only-tecnico-secret-change-in-production-12345'
  );
}
...
export function createTecnicoSession(tecnicoId: number): string {
  const exp = Date.now() + SESSION_TTL_MS;
  const nonce = crypto.randomBytes(8).toString('hex');
  const payload = `${tecnicoId}.${exp}.${nonce}`;
  const sig = crypto
...
export function parseTecnicoSession(
  ...
  const tecnicoId = Number(idStr);
  const exp = Number(expStr);
  if (!Number.isFinite(tecnicoId) || !Number.isFinite(exp)) return null;
  if (Date.now() > exp) return null;
  return { tecnicoId };
}

// ---------------------------------------------------------------------------
// Revalidação (o técnico foi desativado?)
// ---------------------------------------------------------------------------
...
export function isRateLimited(ip: string): boolean {
  ...
  const row = db
    .prepare(
      `SELECT COUNT(*) AS n FROM tecnico_login_attempts
...
export function recordLoginAttempt(
  ...
  const db = getDb();
  db.prepare(
    `INSERT INTO tecnico_login_attempts (ip, tecnico_id, success)
     VALUES (?, ?, ?)`,
  ).run(ip, tecnicoId, success ? 1 : 0);
}
...
export type LoginResult =
  | { ok: true; tecnico: TecnicoRow; token: string }
...
export function attemptLogin(pin: string, ip: string): LoginResult {
  ...
  for (const r of rows) {
    ...
    if (verifyPin(pin, r.pin_hash)) {
      ...
      const token = createTecnicoSession(r.id);
      return {
        ok: true,
        tecnico: { id: r.id, nome: r.nome, pin_ativo: r.pin_ativo },
        token,
      };
    }
  }
...
export const TECNICO_SESSION_TTL_MS = SESSION_TTL_MS;
