...
export async function DELETE(request: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const db = getDb();
    const row = db
      .prepare('SELECT id, url FROM venda_comprovantes WHERE id = ? AND venda_id = ?')
    ...
    // Remove do disco se o arquivo está em /uploads/
    ...
    db.prepare('DELETE FROM venda_comprovantes WHERE id = ?').run(row.id);
