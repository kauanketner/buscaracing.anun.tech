...
const BODY_MAP = {
  rua: 'MOTORCYCLE', offroad: 'MOTORCYCLE', quadriciclos: 'OTHER', infantil: 'OTHER', outros: 'OTHER'
};

app.get('/catalogo.xml', async (_, res) => {
  try {
    const motos = await dbAll('SELECT * FROM motos WHERE ativo=1 ORDER BY destaque DESC, id DESC');
    const items = motos.map(m => {
      const imgUrl = m.imagem ? `${SITE_URL}${m.imagem}` : '';
      const condition = m.condicao === 'nova' ? 'NEW' : 'USED';
      const price = m.preco ? `${Number(m.preco).toFixed(2).replace('.', ',')} BRL` : '';
...
${imgUrl ? `    <image><url>${escapeXml(imgUrl)}</url></image>` : ''}
    ...
    <url>${escapeXml(`${SITE_URL}/moto.html?id=${m.id}`)}</url>
